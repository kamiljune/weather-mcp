/**
 * Weather MCP server factory.
 *
 * Builds a configured MCP `Server` (tool list + dispatch) that is transport
 * agnostic: `src/index.ts` connects one to stdio, `src/http/httpServer.ts`
 * builds one per HTTP request. Upstream services are shared module singletons
 * because they are stateless apart from their own caches; the only per-caller
 * state is the `LocationStore`, which is injected so the HTTP transport can
 * give each API key its own saved-location namespace.
 */
...
/**
 * Execute one of the ChatGPT compatibility tools.
 *
 * `search` geocodes free text into candidate places; `fetch` renders the weather
 * summary for a place id. Errors propagate to the shared dispatch catch, which
 * sanitizes them before they reach the caller.
 */
async function handleChatGptCompatTool(
  name: string,
  args: unknown,
  ...
  if (name === 'search') {
    ...
    if (typeof query !== 'string' || query.trim() === '') {
      throw new Error('The search tool requires a non-empty "query" string.');
    }

    const results = await geocodingService.geocode(query.trim(), CHATGPT_SEARCH_LIMIT);
    return { content: [{ type: 'text', text: formatSearchResults(results) }] };
  }

  if (name === 'fetch') {
    const { latitude, longitude, label } = decodeLocationId(
      (args as { id?: unknown } | undefined)?.id
    );

    const summary = await handleGetWeatherSummary(
      { latitude, longitude },
      noaaService,
      openMeteoService,
      nceiService,
    ...
    return {
      content: [{
        type: 'text',
        text: formatFetchResult({
          id: encodeLocationId(latitude, longitude, label),
          title: label ?? `${latitude}, ${longitude}`,
          text: summary.content.map(block => block.text).join('\n\n'),
          latitude,
          longitude
        })
      }]
...
/**
 * Options for {@link createWeatherServer}.
 */
export interface WeatherServerOptions {
  /**
   * Saved-location store backing the save/list/get/remove tools and the
   * `location_name` parameter. stdio passes the single default store; the HTTP
   * transport passes a per-API-key store so callers never see each other's aliases.
   */
  locationStore: LocationStore;

  /**
   * Also expose the ChatGPT `search`/`fetch` compatibility tools. Off by default:
   * a client that can call the native tools should not see these instead.
   */
  chatgptCompat?: boolean;
}

/**
 * Create an MCP server instance with the enabled tools registered.
 *
 * The returned server is not connected to any transport — the caller connects it.
 */
export function createWeatherServer(options: WeatherServerOptions): Server {
  const { locationStore, chatgptCompat = false } = options;
  ...
  server.setRequestHandler(ListToolsRequestSchema, async () => {
    ...
    const tools: object[] = enabledTools
      .map(toolName => TOOL_DEFINITIONS[toolName])
      .filter(Boolean); // Filter out any undefined tools

    if (chatgptCompat) {
      tools.push(CHATGPT_TOOL_DEFINITIONS.search, CHATGPT_TOOL_DEFINITIONS.fetch);
    }
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
      // The ChatGPT compatibility tools are not part of the ENABLED_TOOLS registry;
      // they are gated by the chatgptCompat option alone.
      if (chatgptCompat && CHATGPT_TOOL_NAMES.includes(name)) {
        return await withAnalytics(name, async () =>
          handleChatGptCompatTool(name, args, locationStore)
        );
      }
