 ...
 *
 * Connects the server built by `createWeatherServer` to the stdio transport,
 * which is how MCP clients spawn this package locally. The HTTP entry point
 * (`src/http/index.ts`) serves the same server over Streamable HTTP.
 */
...
/**
 * Saved locations for the stdio transport live in the single default store
 * (~/.weather-mcp/locations.json) — one machine, one user.
 */
