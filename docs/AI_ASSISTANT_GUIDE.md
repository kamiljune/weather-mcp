...
│   ├── oauth.ts             # Auth0 JWT verification + Garmin-backed user authorization
│   ├── tenantAliases.ts     # Garmin slug → legacy Weather tenant directory mapping
...
- **Hosted HTTP accepts Auth0 OAuth only.** Verify JWT signature/issuer/audience/expiration locally, then ask Garmin's private authorization endpoint whether the same `sub` is an active `/connect` user. Fail closed when that service is unavailable; never log the token or `sub`.
