...
- **2026-09-21 HTTP 鉴权改成 IdP 中立：只配 issuer，JWKS 和签名算法从 IdP 读（为换 Logto）**
  - 变更：`WEATHER_AUTH0_DOMAIN` / `WEATHER_AUTH0_AUDIENCE` 改名为 `WEATHER_OIDC_ISSUER`（完整 issuer URL）/ `WEATHER_OIDC_AUDIENCE`，旧名还在时启动报错。
    ...
    discovery 失败返回 unavailable 且不缓存失败。PRM 的 `authorization_servers` 直接用 issuer。
  - 原因：原来按 Auth0 写死了 `https://<域名>/`、`/.well-known/jwks.json` 和 RS256；Logto 的 issuer 不带尾斜杠、JWKS 在 `/oidc/jwks`、签 ES384。
  - 影响：`src/config/http.ts`、`src/http/oauth.ts`、`httpServer.ts`、`index.ts` 及三份测试；新增 RS256/ES384 双算法、HS256 拒收、discovery 说谎/宕机后重试的用例，
    拿掉 ES384 或 issuer 校验各自会红。全量 `npm test` 有 5 个与本次无关的既有失败（超时默认值、12 月 31 日），改动前后一致。
    服务器 `.env` 要同步改名。
...
- **Streamable HTTP transport — run the server as a hosted service** (`npm run start:http`, `dist/http/index.js`) - Until now the only way to reach these tools was a locally spawned stdio process. A second entry point now serves the same server over MCP Streamable HTTP so hosted assistants — Claude custom connectors, ChatGPT connectors — can reach it at a URL like `https://weather.example.com/mcp/<api-key>`. **The stdio path is untouched**: `src/index.ts` is now a thin entry point over a new `createWeatherServer()` factory (`src/server/weatherServer.ts`), which holds the unchanged TOOL_DEFINITIONS registry and dispatch, and the full unit suite passes **unedited** as the lock on that.
  ...
  The transport runs **stateless** (`sessionIdGenerator: undefined`): each POST builds a server, answers, and tears it down, so restarts and multiple replicas are invisible to clients and nothing outlives a request except the shared upstream caches. Responses default to a single `application/json` body rather than SSE (`WEATHER_HTTP_JSON_RESPONSE`), because a buffering reverse proxy breaks SSE and these are one-shot request/response calls either way.

  **Authentication is an API key in the URL path** (`POST /mcp/<key>`), the one form every remote-MCP client UI can express, with `Authorization: Bearer <key>` and `?key=` accepted as alternatives. The key-in-URL trade-off is disclosed rather than glossed over, with mitigations, in `docs/DEPLOY_HTTP.md`. Keys follow the project's existing key-in-URL discipline in reverse: only their SHA-256 is retained, lookup is a hash-map probe so no code path branches on how much of a key matched, a key shorter than 24 characters is refused **at startup** rather than silently accepted, and the raw key never reaches a log line, an error message, or a filesystem path — everything downstream identifies a caller by a non-secret tenant id and label.

  **Saved locations are namespaced per tenant.** stdio serves one person on one machine, so one `~/.weather-mcp/locations.json` is right; over HTTP one process answers every caller and saved locations are home and work addresses. Each tenant gets `WEATHER_DATA_DIR/<tenant>/locations.json`, and `LocationStore` gained an optional `disclosePath` flag (default **true**, so stdio output is byte-identical) that the HTTP transport turns off — the rendered `*Storage location: …*` line named a server directory, which is a caller's business on their own laptop and nobody's business on a shared service.

  Also included: per-tenant token-bucket rate limiting with `Retry-After` (`WEATHER_HTTP_RATE_LIMIT`, default 120/min — what stands between a runaway client and the free upstream APIs blocking your IP), a request body cap answered with a clean 413 rather than a reset socket, optional DNS-rebinding protection (`WEATHER_HTTP_ALLOWED_HOSTS`/`_ORIGINS`), an unauthenticated `/healthz`, graceful SIGTERM shutdown, a multi-stage `Dockerfile` + `docker-compose.yml`, and `docs/DEPLOY_HTTP.md` covering key generation, 1Panel/nginx/Caddy reverse-proxy configuration (including keeping the key out of access logs), and client-by-client connection steps.

- **Access is organised by tenant, and the key file reloads without a restart** - The first cut keyed everything on the API key itself: the saved-location directory was a hash prefix of the key, and the accepted set was read once from `WEATHER_API_KEYS` at startup. Both were wrong in the same way — they conflated *who is calling* with *which secret they presented*. Rotating a key silently abandoned that person's saved locations, and adding or revoking anyone meant bouncing the service on everyone else.

  Identity is now a **tenant**: an operator-chosen id owning one or more keys. The id names the data directory (`WEATHER_DATA_DIR/<tenant>/locations.json`) and never changes, so a key can be rotated by adding the new one, moving clients over, and removing the old one — with no data loss and no downtime. One person can hold a Claude key and a ChatGPT key over the same saved locations. A leaked key is revoked on its own. Rate-limit budgets are per tenant, not per key. Because the id becomes a path segment it is validated to `^[a-z0-9][a-z0-9_-]{0,63}$` and rejected otherwise — turning operator input into a filesystem path is exactly the mistake the key-handling rules exist to prevent.

  `WEATHER_API_KEYS_FILE` points at a JSON document (`{"tenants":[{"id","label","keys":[]}]}`) that is re-read on change — mtime polling every `WEATHER_API_KEYS_RELOAD_SECONDS` (default 10, `0` disables) rather than inotify, because the file is typically a Docker bind mount where filesystem events are unreliable; `SIGHUP` forces an immediate reload. **The load-bearing rule is that a bad reload never takes the service down**: the registry is swapped only when a new document parses and validates *completely*, so a syntax error, a truncated write, or a deleted file leaves the last good key set serving and logs `API key reload failed` — handing every caller a 401 over a stray comma would be far worse than briefly running on stale keys. Live-verified end to end: a tenant added to the file authenticated ~2 s later with no restart and no disturbance to the existing tenant, `SIGHUP` on a deliberately truncated file kept both tenants working, and no key material appeared anywhere in the logs.

  `WEATHER_API_KEYS` still works and is now equivalent: entries sharing a name (`kamil:keyA,kamil:keyB`) become one tenant with two keys. A bare unlabelled key still authenticates, but its tenant id is derived from the key, so rotation starts an empty namespace — the server warns about that at startup. `docker-compose.yml` mounts `./config` as a **directory** rather than bind-mounting the file: a single-file bind mount pins an inode, most editors replace the inode on save, and reload would silently stop working.

- **ChatGPT `search`/`fetch` compatibility tools, off by default** (`WEATHER_CHATGPT_COMPAT=true`) - ChatGPT's deep-research connector calls only two tools by contract: `search`, returning `{id, title, url}` documents, and `fetch`, expanding one id into `{id, title, text, url, metadata}`. Mapped onto this server a "document" is a place: `search` geocodes free text into candidate locations, `fetch` renders that location's `get_weather_summary`. They are **added alongside** the native tools, never instead of them, and are **off by default** — a client that can call the native 17 should not be handed a strictly worse pair. Location ids are opaque (`geo:<lat>,<lon>:<encoded label>`), round-trip a label containing commas, colons or slashes, are validated through the existing coordinate validators, and reject anything that is not a coordinate pair.
